...
def build_up_recurrent_connections(NTWK, SEED=1, verbose=False):
    ...
    for ii, jj in itertools.product(range(len(NTWK['POPS'])), range(len(NTWK['POPS']))):
        if (NTWK['M'][ii,jj]['pconn']>0) and (NTWK['M'][ii,jj]['Q']!=0):
            ...
            source_pop = NTWK['NEURONS'][ii]['name']
            target_pop = NTWK['NEURONS'][jj]['name']

            SYNAPSES_EQUATIONS, ON_EVENT, P =\
                    get_syn_onevent_params(source_pop, target_pop, Model)

            CONN[ii,jj] = brian2.Synapses(NTWK['POPS'][ii], NTWK['POPS'][jj], 
            ...
            if ii==jj: # need to take care of no autapse
                ...
                i_rdms = np.concatenate([\
                                np.random.choice(np.arange(NTWK['POPS'][ii].N), 
                                                 N_per_cell)\
